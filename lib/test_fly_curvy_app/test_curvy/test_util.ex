defmodule TestCurvy.Util do
  ...
  def inv_op(lm, hm, low, high) when low > 1 do
    r = div(high, low) |> dbg()
    first = hm - lm * r |> dbg()
    second = lm |> dbg()
    third = high - low * r |> dbg()
    fourth = low |> dbg()
    inv_op(hm - lm * r, lm, high - low * r, low) |> dbg()
  end

  def inv_op(lm, _hm, _low, _high), do: lm
